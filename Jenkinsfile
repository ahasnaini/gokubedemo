...
sh('docker images --quiet --filter=dangling=true | xargs --no-run-if-empty docker rmi')
 ...
 stage('Trigger Deploy'){
     ...
     if(env.JOB_NAME=="GoKubeDemo/Development")
     {
         ...
         sh('export PATH=$PATH:/var/jenkins_home/ && kubectl --context=dev.kubernetes.asadali.net set image deployment/demo-app-deployment demoapp=asadali/gokubedemo:1.0.$BUILD_NUMBER.$JOB_BASE_NAME')
     }
     }
    }
/*stage('Deploy') {
